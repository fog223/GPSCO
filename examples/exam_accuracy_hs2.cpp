/**
 *  \file   exam_accuracy_park.cpp
 *  \brief  Evaluation of the accuracy of the algorithm GPSCO(HS1)
 *  \author fog
...
int main(int argc, char** argv)
{
	...
	for (const auto& pair : pair_regis_idx)
	{
		...
		if ((pair.first == 2 && pair.second == 3)
			...
			|| (pair.first == 4 && pair.second == 7)
			|| (pair.first == 6 && pair.second == 7)
			|| (pair.first == 7 && pair.second == 8)
			|| (pair.first == 8 && pair.second == 9)
			|| (pair.first == 8 && pair.second == 13)
